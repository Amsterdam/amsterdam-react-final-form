...
const UnboundRadioFields:React.FC<Props> = ({
  name, horizontal, label, extraLabel, extraLabelAlign, hint, error, position,
  align, options, value, onChange, tooltip, ...restProps
}) => {
        ...
        { Object
          ...
          .map(([key, label]) => (
            <AscLabel key={key} htmlFor={`${ name }_${ key }`} label={label}>
              <Radio {...restProps} onChange={handleChange} error={!!error} id={`${ name }_${ key }`} data-testid={key} name={name} value={key} checked={ value === key } />
            </AscLabel>
          )) }
      </RadioGroup>
    </ComposedField>
